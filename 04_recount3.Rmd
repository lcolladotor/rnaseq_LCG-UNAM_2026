...
Después tenemos que identificar un estudio de interés y determinar si queremos acceder a la información a nivel de genes, exones, etc. Sabiendo el estudio de interés, podemos descargar los datos usando la función `create_rse()` como mostramos a continuación. `create_rse()` tiene argumentos con los cuales podemos especificar la **anotación** que queremos usar (las opciones dependen del organismo).
...
```{r "tranform_counts"}
## Convirtamos las cuentas por nucleótido a cuentas por lectura
...
* (opcional) Crea tu cuenta gratis de <https://www.shinyapps.io/> y comparte tu visualización de los datos usando `iSEE` de esa forma.
  - OJO: Necesitarás una cuenta pagada para poder tener acceso a computadoras con suficiente memoria para los datos que tenemos. O usa un set de datos más pequeño.
...
* Autores de recount2 y 3 en redes sociales:
...
* Ejemplos de posts de la comunidad
